...
function parseFormat(format) {
  ...
  const [, fill, align2, sign, symbol, zero, width, comma, precision, trim, type] = match;
  return {
    fill,
    align: align2,
    sign,
...
function numberFormat(format) {
  ...
  const { fill, align: align2, sign = "-", symbol, zero, width, comma, type, prefix = "", suffix = "", precision } = options;
  let { trim } = options;
  ...
  return (n) => {
    ...
    if (width != null && !isNaN(width)) {
      result = addPadding(result, width, fill ?? zero, align2);
    }
...
function addPadding(numString, width, fill = " ", align2 = ">") {
...
function niceTicksDomain(start, end) {
  ...
  for (const multiplier of TickMultipliers) {
    ...
    const error2 = 1 - extent / Math.abs(d1 - d0);
    if (minError > error2) {
      minError = error2;
      ticks = [d0, d1];
    }
